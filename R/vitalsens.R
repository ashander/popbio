...
#'@return A dataframe with vital rate estimates, sensitivities, and
#'elasticities.
#'@note The element expressions should return the actual matrix element
#'estimates after evaluating the variables using \code{\link{eval}} below.
#'
#'\code{A<-sapply(elements, eval, vitalrates, NULL)}
#'
#'In addition, these expressions should be arranged by rows so the following
#'returns the projection matrix.
#'
#'\code{matrix(A, nrow=sqrt(length(elements)), byrow=TRUE)}
#'@author Chris Stubben.  Based on code posted by Simon Blomberg to R-help
#'mailing list.
