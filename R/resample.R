...
#'
#'Resample a projection matrix using a multinomial distribution for transitions
#'and a log normal distribution for fertilities
#'
...
#'in each corresponding stage class in \code{n}.
#'
#'The fertility rates are sample from a Log Normal distribution using the
#'\code{\link{lnorms}} function.  The variance can be a single value which is
#'applied to all rates, or vector of different values to apply to each rate.
#'In this case, the values are recycled to match the number of non-zero
#'fertilities.
#'
#'@param A a projection matrix
#'@param n either a stage vector with the number of transitions to sample in
#'each column or a single value that is applied to all columns
